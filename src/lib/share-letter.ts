/* Monthly-letter section posters (/api/share/letter/[slug]?section=...):
   ...
   hand-written); poster chrome and assembled fact labels follow the
   explicit locale query parameter. Dev
...
export function decisionKindLabel(
  ...
  if (locale === "en") {
    return kind === "work" ? "SELECTED WORK" : kind === "post" ? "SELECTED THREAD" : "GOVERNANCE";
  }
  return kind === "work" ? "精选作品" : kind === "post" ? "精选讨论" : "治理公示";
}
...
export function buildLetterShareSnapshot(
  ...
  return {
    ...
    decisions: issue.decisions
      ...
      .map((d) => ({
        kind: d.kind,
        kindLabel: decisionKindLabel(d.kind, locale),
...
export function letterSnapshotFromResult(
  ...
  return result ? buildLetterShareSnapshot(result.issue, section, locale) : null;
}
...
export async function getLetterShareSnapshot(
  ...
  return letterSnapshotFromResult(
    await getAssembledIssue(s, locale, { stats: await getCachedMonthlyStatsSnapshot() }),
    section,
    locale,
  );
}
  ...
  return {
    ...
    decisions: issue.decisions
      ...
      .map((d) => ({
        ...
        kindLabel: locale === "zh" ? MOCK_DECISION_LABEL[d.kind] : decisionKindLabel(MOCK_DECISION_KIND[d.kind], locale),
