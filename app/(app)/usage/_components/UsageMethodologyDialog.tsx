  ...
  return (
    <>
      <button
        ...
        className={
          compact
            ? "inline-flex size-7 shrink-0 items-center justify-center text-grey/70 hover:text-ui-blue focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue"
            : "inline-flex min-h-11 w-full items-center justify-center gap-1.5 rounded-lg px-3 font-mono text-xs text-grey hover:text-paper focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue sm:w-auto"
        }
      >
        ...
        <div className="max-h-[calc(86vh-66px)] overflow-y-auto px-5 py-5 text-xs leading-relaxed text-grey sm:px-6">
            ...
            <section>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div className="border border-line bg-card p-3">
                  <div className="font-mono text-xs text-paper">TOTAL TOKENS</div>
                  <p className="mt-1.5">
                    {zh
                ...
                <div className="border border-line bg-card p-3">
                  <div className="font-mono text-xs text-paper">PEAK TOKENS</div>
                  <p className="mt-1.5">
                    {zh
                ...
                <div className="border border-line bg-card p-3">
                  <div className="font-mono text-xs text-paper">WEEKLY / HEATMAP</div>
                  <p className="mt-1.5">
                    {zh
          ...
          {sections.includes("pricing") && (
            <section className={sections[0] === "pricing" ? "" : "mt-7 border-t border-line pt-6"}>
              ...
              <code className="mt-3 block overflow-x-auto border border-line bg-card p-3 font-mono text-xs text-paper">
              ...
              <div className="mt-4 overflow-x-auto border border-line">
                <table className="w-full min-w-[1180px] border-collapse font-mono text-xs">
                  ...
                  <tbody>
                    {pricingMatches.length === 0 ? (
                      ...
                      pricingMatches.map((row) => (
                        /* The key must mirror the query layer's
                           pricingMatchMap equality fields: one (source,
                           model) can split into multiple rows by
                           canonical/provider; missing either field
                           duplicates keys. */
                        <tr key={`${row.source}-${row.model}-${row.modelCanonical}-${row.modelProvider}-${row.contextTier}-${row.processingTier}-${row.version}-${row.effectiveFrom}`} className="border-t border-line">
                          ...
                          <td className="px-3 py-2 text-grey">
                            ...
                            <span className="block text-xs">{row.processingTier || "standard"}</span>
                          </td>
                          ...
                          <td className="px-3 py-2 text-grey">
                            ...
                            {row.pricingSourceUrl ? (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="underline decoration-grey/50 underline-offset-2 hover:text-ui-blue"
                              >
                            ...
                            {row.assumptions.length > 0 ? (
                              <span className="block text-xs text-status-warn-fg">
            ...
            <section className={sections[0] === "duration" ? "" : "mt-7 border-t border-line pt-6"}>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div className="border-l-2 border-blue pl-3">
                  <div className="font-mono text-xs text-paper">{zh ? "活跃时长" : "ACTIVE TIME"}</div>
                  <p className="mt-1.5">
                    {zh
          ...
          {sections.includes("changes") && (
            <section className={sections[0] === "changes" ? "" : "mt-7 border-t border-line pt-6"}>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                <div>
                  <div className="font-mono text-xs text-paper">{zh ? "当前周期" : "CURRENT"}</div>
                ...
                <div>
                  <div className="font-mono text-xs text-paper">{zh ? "上一等长周期" : "PREVIOUS EQUAL PERIOD"}</div>
