    ...
    <div className={`${compact ? "p-4" : "p-5 sm:p-6"} border border-status-danger/40 bg-card`}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 font-mono text-xs text-grey">{reference}</p>
          <button
            type="button"
            disabled={pending}
            onClick={() => startTransition(() => router.refresh())}
            className="mt-3 inline-flex min-h-11 items-center gap-2 rounded-lg border border-line px-3 font-mono text-xs text-paper hover:border-ui-blue focus-visible:outline focus-visible:outline-2 focus-visible:outline-blue disabled:cursor-wait disabled:opacity-50"
          >
