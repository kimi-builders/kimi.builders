        ...
        <div>
          ...
          <p className="mt-1.5 text-xs leading-relaxed text-grey/80">
            {kind === "letter"
              ...
              : locale === "zh"
                ? '可用字段:cover(封面,站内路径或 https 图片,列表卡左列)、chapter(章,单篇教程用,四章词表见 src/lib/kb-chapters.ts)、series(系列 slug,须在册 src/lib/learn-series.ts,可留空)、products(产品透镜,≤3,slug 见 src/lib/kb-products.ts,主产品在前)、roles(职业透镜,≤3,slug 见 src/lib/kb-roles.ts)、video({provider:"bilibili"/"youtube",id})、deck(演示稿链接)、durationMin(分钟)、scenario(场景,动词开头)、tags(≤5 个)、resources([{label,url,kind}] ≤8 条,kind 可选 official/resource/prompt/skill/file)、aiNote(AI 参与披露)。公约:文稿是 canonical,三媒体齐备;每期必须有「跟着做完」的毕业动作——做完拥有什么,而不是知道了什么。'
                : 'Keys: cover (image path or https URL, list card), chapter (standalone guides only; four chapters in src/lib/kb-chapters.ts), series (registered in src/lib/learn-series.ts, optional), products (≤3 lens slugs, src/lib/kb-products.ts, primary first), roles (≤3, src/lib/kb-roles.ts), video ({provider:"bilibili"/"youtube",id}), deck (slides link), durationMin, scenario (verb-first), tags (≤5), resources ([{label,url,kind}] ≤8, kind = official/resource/prompt/skill/file), aiNote. Rule: the script is canonical, all three media present; every piece must end in a followable action — what readers walk away with, not what they now know.'}
          </p>
        </div>
      )}
